...
class _MLTRADER(Strategy):
    ...
    def position_sizing(self):
        ...
        if cash <= 0 or last_price <= 0:
            self.log("Cash or last price is non-positive. Cannot calculate position size.", level='ERROR')
    ...
    def dynamic_risk_management(self, last_price, risk_tolerance=0.02, profit_margin=0.10, cap_limit=0.30):
        """
    ...
    def on_trading_iteration(self):
        ...
        if cash > 0:
            if sentiment == 'positive' and probability > .999:
                ...
                take_profit, stop_loss = self.dynamic_risk_management(last_price)
